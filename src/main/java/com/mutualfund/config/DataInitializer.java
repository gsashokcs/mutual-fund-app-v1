...
public class DataInitializer implements CommandLineRunner {
    ...
    /**
     * Creates the default admin user if it doesn't exist in the database. Uses credentials: username=admin, password=admin123, role=ADMIN
     */
    private void initializeAdminUser() {
        String adminUsername = "admin";

        if (!userRepository.existsByUsername(adminUsername)) {
            User admin = User.builder().username(adminUsername).password(passwordEncoder.encode("admin123")).role(User.Role.ADMIN).build();
    ...
    /**
     * Loads popular Indian mutual funds with NAV data from yesterday. Only creates funds if the table is empty.
     */
    private void initializeMutualFunds() {
        ...
        createMutualFundWithNav("HDFC Balanced Advantage Fund", new BigDecimal("368.92"), yesterday);
        createMutualFundWithNav("ICICI Prudential Bluechip Fund", new BigDecimal("98.76"), yesterday);
        ...
        createMutualFundWithNav("HDFC Mid-Cap Opportunities Fund", new BigDecimal("178.45"), yesterday);
    ...
    private void createMutualFundWithNav(String name, BigDecimal navValue, LocalDate navDate) {
        ...
        Nav nav = Nav.builder().fundId(savedFund.getFundId()).nav(navValue).navDate(navDate).deleted(false).build();
