...
public class UserService implements IUserService {
    ...
    public UserResponse registerUser(UserRegistrationRequest request) {
        ...
        if (userRepository.existsByUsername(request.getUsername())) {
            throw new BusinessException(ErrorCode.DUPLICATE_USER, "Username already exists: " + request.getUsername());
        }

        User user = User.builder().username(request.getUsername()).password(passwordEncoder.encode(request.getPassword())).role(User.Role.USER).build();
    ...
    public UserResponse createUser(com.mutualfund.model.request.AdminUserCreationRequest request) {
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.VALIDATION_ERROR, "Invalid role: " + request.getRole());
        }

        User user = User.builder().username(request.getUsername()).password(passwordEncoder.encode(request.getPassword())).role(role).build();

        User savedUser = userRepository.save(user);
        log.info("User created successfully by admin with ID: {} and role: {}", savedUser.getId(), savedUser.getRole());
    ...
    /**
     * Retrieves a user by their ID. Regular users can only access their own profile. Admin users can access any profile.
     *
     * @param userId
     ...
     * @throws BusinessException
     *             if user attempts to access another user's profile
     */
    @Transactional(readOnly = true)
    public UserResponse getUserById(Long userId) {
        ...
        User user = userRepository.findById(java.util.Objects.requireNonNull(userId, "User ID cannot be null")).orElseThrow(() -> new ResourceNotFoundException(ErrorCode.USER_NOT_FOUND, "User not found with ID: " + userId));
    ...
    public void deleteUser(Long userId) {
        ...
        if (!userRepository.existsById(userId)) {
            throw new ResourceNotFoundException(ErrorCode.USER_NOT_FOUND, "User not found with ID: " + userId);
        }
    ...
    public User findByUsername(String username) {
        return userRepository.findByUsername(username).orElseThrow(() -> new ResourceNotFoundException(ErrorCode.USER_NOT_FOUND, "User not found: " + username));
    }

    private UserResponse mapToResponse(User user) {
        return UserResponse.builder().id(user.getId()).username(user.getUsername()).role(user.getRole().name()).build();
    }
}
